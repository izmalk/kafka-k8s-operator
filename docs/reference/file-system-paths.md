...
myst:
  html_meta:
    description: "File system paths reference for Charmed Apache Kafka K8s including configuration, logs, data directories, and workload paths."
---
...
|:--------------------:|:------------------------------------------------:|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|:----------------------------------------:|
|         `BIN`        |      `/opt/kafka`     |                                                                   Binary files for the Charmed Apache Kafka K8s distribution, bundled in the `charmed-kafka` OCI image.                                                                   |                 read-only                |
...
In the rest of this page, the path variables (`$CONF`, `$BIN`, `$LOGS` and `$DATA`)
are used as shorthand for the paths in the table above.
The same layout is used for the Cruise Control balancer workload, under
`/opt/cruise-control` (binaries), `/etc/cruise-control` (configuration),
`/var/log/cruise-control` (logs) and `/var/lib/cruise-control` (data).
...
- **`$CONF/client-keystore.p12` + `$CONF/client-truststore.jks`** - the Java keystore and truststore used for client SSL encryption
  - Passwords to the keystore and truststore are stored in Juju secrets
- **`$CONF/log4j.properties`** - the Log4j configuration of the Apache Kafka service
- **`$CONF/tools-log4j.properties`** - the Log4j configuration used by the `bin/*.sh` utility commands
- **`$CONF/jmx_prometheus.yaml`** - the configuration of the JMX exporter for the broker
- **`$CONF/cruisecontrol.properties`**, **`$CONF/cruise_control_jaas.conf`**, **`$CONF/capacityJBOD.json`** - the Cruise Control balancer configuration files
